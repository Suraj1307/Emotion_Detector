...
def _load_model_compat(model_path: str):
    ...
    return _rebuild_model_from_keras_archive(model_path)
...
def load_bert_tokenizer():
    ...
    return AutoTokenizer.from_pretrained(MODEL_REPO_ID)
...
def initialize_pipeline() -> Tuple[keras.Model, any, LabelEncoder, int]:
    ...
    dummy_ids = np.zeros((1, max_len), dtype=np.int32)
    dummy_mask = np.ones((1, max_len), dtype=np.int32)
    model({"input_ids": dummy_ids, "attention_mask": dummy_mask}, training=False)
